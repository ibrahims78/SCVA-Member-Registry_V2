...
> **تاريخ المراجعة الأصلية:** 29 نيسان/أبريل 2026.
> **آخر تحديث لحالة البنود:** 6 أيار/مايو 2026 (الإصدار v2.5).
...
## 📊 حالة البنود المحدَّثة (v2.5 — مايو 2026)

يعرض هذا القسم الوضع الراهن لجميع بنود المراجعة الأصلية بعد مراحل الإصلاح الست.

### 🔴 المشاكل الحرجة

| الرمز | الوصف المختصر | الحالة |
|-------|--------------|--------|
| ح-1 | كلمة مرور افتراضية مشفّرة في الكود (`"12345678"`) | ✅ **مُصلَح** — كلمة مرور عشوائية 24 حرفاً تُطبع مرّة بـ `stderr`، و`mustChangePassword: true` يُجبر التغيير |

### 🟠 المشاكل الأمنية العالية

| الرمز | الوصف المختصر | الحالة |
|-------|--------------|--------|
| ع-1 | غياب الحماية من القوة الغاشمة على `/api/login` | ✅ **مُصلَح** — `express-rate-limit`: 10 محاولات / 15 دقيقة / IP |
| ع-2 | لا يوجد `helmet` ولا رؤوس أمان HTTP | ✅ **مُصلَح** — `helmet()` مُطبَّق على جميع الاستجابات |
| ع-3 | لا توجد حماية CSRF مع استعمال الجلسات | ✅ **مُصلَح** — `sameSite: strict` + فحص `Origin`/`Referer` في الإنتاج |
| ع-4 | مسار PDF يعتمد puppeteer بخطر SSRF | ✅ **مُخفَّف** — رسالة 503 واضحة عند غياب Chromium، proxy cookie مُصلَح. استبدال كامل بـ pdfkit لا يزال توصية مستقبلية. |

### 🟡 المشاكل الأمنية المتوسطة والمنخفضة

| الرمز | الوصف المختصر | الحالة |
|-------|--------------|--------|
| م-1 | حدّ حجم JSON غير محدّد | ✅ **مُصلَح** — 5MB لـ JSON، 1MB لـ urlencoded |
| م-2 | كلمات مرور افتراضية في `docker-compose.yml` | ✅ **موثَّق** — تعليق واضح أنّ الملف للتطوير فقط |
| م-3 | `window.location.href` بدل `useLocation()` في `Members.tsx` | ⚠️ **مفتوح** — تحسين صغير مؤجَّل |
| م-4 | `Dockerfile` يحوي Chromium (~400MB) | ⚠️ **مفتوح** — مرتبط بقرار استبدال puppeteer |

### 🔵 أخطاء TypeScript

| الرمز | الوصف المختصر | الحالة |
|-------|--------------|--------|
| TS-1 | 8 أخطاء نوع على حقول `null` في `Home.tsx`, `MemberDetails.tsx`, `Members.tsx` | ✅ **مُصلَح** — فحوص دفاعية `?? ""` و`?? "—"` مُضافة في جميع المواقع |

### 🟢 الأداء

| الرمز | الوصف المختصر | الحالة |
|-------|--------------|--------|
| أ-1 | استعلامات N+1 في `/api/members` و`/api/backup` | ✅ **مُصلَح** — `getSubscriptionsByMemberIds(ids)` بـ `inArray()` واحدة |
| أ-2 | `staleTime: Infinity` على مستوى عام | ✅ **مُصلَح** — `staleTime: 30 * 1000` (30 ثانية) |
| أ-3 | `getQueryFn` يكسر المفاتيح الهرمية مع كائنات | ⚠️ **مفتوح** — الكود الحالي يستخدم URL كاملة في العنصر الأول؛ المشكلة هامشية |

### ⚪ جودة الكود والبنية

| الرمز | الوصف المختصر | الحالة |
|-------|--------------|--------|
| ج-1 | ملفات ضخمة (+600-900 سطر) | ⚠️ **مفتوح** — تحسين تدريجي مع إضافة الميزات |
| ج-2 | `fetch` مباشر في `Settings.tsx` بدل `apiRequest` | ⚠️ **مفتوح** — تحسين مؤجَّل |
| ج-3 | أنواع `any` متناثرة (14 موقعاً) | ⚠️ **مفتوح جزئياً** — المستوردات الرئيسية نُوِّعت؛ بعض `any` لا يزال |
| ج-4 | `server/seed.ts` غير موجود | ✅ **مُصلَح** — `seed.ts` موجود ويعرض ملخّص DB |
| ج-5 | `console.log` في كود الإنتاج | ✅ **مُصلَح** — سجلّات موحَّدة عبر دالة مركزية |
| ج-6 | تكرار `params?.id` في `AddMember.tsx` | ✅ **مُصلَح** — استخراج المتغيّر مرّة واحدة |
| ب-1 | منطق العمل في `routes.ts` بدل طبقة خدمة | ⚠️ **مفتوح** — إعادة هيكلة كبيرة مؤجَّلة |
| ب-2 | `MembersContext` يكرّر حالة TanStack Query | ✅ **مُخفَّف** — `MembersContext` أصبح غلافاً رفيعاً فوق React Query بدون حالة مكرَّرة |
| ب-3 | الترجمات في كائن واحد ضخم | ⚠️ **مفتوح** — تحسين معماري مؤجَّل |
| ب-4 | لا توجد اختبارات مؤتمتة | ⚠️ **مفتوح** — لا اختبارات بعد |

### 📄 الوثائق والنشر

| الرمز | الوصف المختصر | الحالة |
|-------|--------------|--------|
| و-1 | `README.md` يدّعي ما لا يفعله الكود | ✅ **مُصلَح** — `README.md` محدَّث كلّياً |
| و-2 | `replit.md` لا يعكس آخر التغييرات | ✅ **مُصلَح** — `replit.md` محدَّث لـ v2.5 |

### 📋 ملخّص الحالة

| التصنيف | مُصلَح / مُخفَّف | مفتوح |
|---------|--------------|------|
| حرج | 1 / 1 | 0 |
| أمني عالٍ | 4 / 4 | 0 |
| أمني متوسط/منخفض | 2 / 4 | 2 |
| TypeScript | 1 / 1 | 0 |
| أداء | 2 / 3 | 1 |
| جودة / بنية | 5 / 11 | 6 |
| وثائق | 2 / 2 | 0 |
...
> **الجاهزية للإنتاج:** ✅ **جاهز** — جميع البنود الحرجة والأمنية العالية مُعالَجة. البنود المفتوحة تحسينات جودة ومعمارية لا تؤثّر على الأمان أو الاستقرار.
